...
mouse_data <- tibble(
  ...
  "num_TD_Tom_pos_per_slice_ctx" = rnorm(cohort_size, 200, 20),
  "num_488_pos_per_slice_ctx" = rnorm(cohort_size, 200, 20),
  "percent_coloc_ctx" = 0,
  "num_coloc_ctx" = 0,
  "num_TD_Tom_pos_per_slice_hip" = rnorm(cohort_size, 200, 20),
  "num_488_pos_per_slice_hip" = rnorm(cohort_size, 200, 20),
  ...
  "num_coloc_hip" = percent_coloc_hip * 100
   ...
   for(i in 1:nrow(mouse_data)) {
    mouse_data$percent_coloc_ctx[i] <- abs(rnorm(1, (mouse_data$Freezing[i] / 2 *100), 7))
...
ggqqplot(mouse_data$Freezing) + ylab("Percent freezing time") + xlab("Theoretical percent freezing time")
...
shapiro.test(mouse_data$Freezing)
...
ggplot(coloc_by_region, aes(Percent_freezing, Engram_coloc_percent, fill = Brain_region))  + geom_smooth(method = "lm", colour = "lightsteelblue3") +
scale_fill_manual(values=wes_palette(n=2, name="GrandBudapest2")) + xlab("Percent freezing time") + ylab("Perent colocalized cells") + geom_point( aes(shape = Brain_region))



```

Cortical and Hippocampal correlations - r vals linear fit
```{r}


cor_cortex <- filter(coloc_by_region, Brain_region == "Cortex")
cor_hip <- filter(coloc_by_region, Brain_region == "Hippocampus")
cor(cor_cortex$Percent_freezing, cor_cortex$Engram_coloc_percent)
cor(cor_hip$Percent_freezing, cor_hip$Engram_coloc_percent)
...
engram_counts %>% 
  ...
  filter(Distrobution != "All Cells") %>%
  mutate(Distrobution = factor(Distrobution, levels=c("Bottom 25 Percent", "Top 25 Percent"))) %>%
...
engram_counts %>%
  ...
  mutate(Engram_labels = factor(Engram_labels, levels=c("Early Engram Cells", "Late Engram Cells", "Colocalized Engram Cells"))) %>%
  ggplot( aes(Engram_cell_counts, fill = Distrobution)) +
         geom_violin(aes(Distrobution, Engram_cell_counts)) + geom_point(aes(Distrobution, Engram_cell_counts, alpha = 3))  + facet_wrap(vars(Engram_labels)) + scale_y_continuous(breaks = NULL) + scale_fill_manual(values=wes_palette(n=4, name="GrandBudapest2")) + xlab("Engram Cell Count (per slice)") + theme(axis.text.x=element_text(angle=90, hjust=1))
...
t-test on engram cells 
```{r}

mouse_data_bot <- filter(mouse_data, Distrobution == "Bottom 25 Percent")
mouse_data_top <- filter(mouse_data, Distrobution == "Top 25 Percent")

wilcox.test(mouse_data_bot$num_TD_Tom_pos_per_slice_ctx, mouse_data_top$num_TD_Tom_pos_per_slice_ctx)
wilcox.test(mouse_data_bot$num_488_pos_per_slice_ctx, mouse_data_top$num_488_pos_per_slice_ctx)
wilcox.test(mouse_data_bot$num_coloc_ctx, mouse_data_top$num_coloc_ctx)
...
ggplot(mouse_data, aes(Freezing, percent_coloc_ctx)) + 
  geom_point() + 
  geom_smooth(method = "lm", colour = "plum4") + xlab("Freezing (percent time)") + ylab("Percent of cells colocalized")

ggplot(mouse_data, aes(Freezing, Weight)) + 
  geom_point() + 
  geom_smooth(method = "lm", colour = "plum4") + xlab("Freezing (percent time)") + ylab("Mouse body weight")

ggplot(mouse_data, aes(Freezing, age_in_months)) + 
  geom_point() + 
  geom_smooth(method = "lm", colour = "plum4") + xlab("Freezing (percent time)") + ylab("Mouse age (in months)")


```


corr for these plots 
```{r}
cor(mouse_data$percent_coloc_ctx, mouse_data$Freezing)
cor(mouse_data$Weight, mouse_data$Freezing)
cor(mouse_data$age_in_months, mouse_data$Freezing)
...
lm_coloc <- lm( mouse_data$Freezing ~  mouse_data$percent_coloc_ctx)
lm_weight <- lm(mouse_data$Weight ~ mouse_data$percent_coloc_ctx)
lm_age <- lm(mouse_data$age_in_months ~ mouse_data$percent_coloc_ctx)
...
plot(lm_coloc$residuals)

plot(mouse_data$percent_coloc_ctx, lm_coloc$residuals)
abline(h=0, col="red")

plot(mouse_data$Weight, lm_coloc$residuals)
abline(h=0, col="red")

plot(mouse_data$age_in_months, lm_coloc$residuals)
abline(h=0, col="red")
